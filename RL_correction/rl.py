...
class PPO():
    ...
    def act_and_value(self, feats, actions_seq, actor_critic: ActorCritic):
        ...
        feats_win = feats
        acts_win = actions_seq

        if S > W:
            feats_win = feats[-W:, :]
            acts_win = actions_seq[-W:]

        feats_win = feats_win.unsqueeze(0)
        acts_win = acts_win.unsqueeze(0).unsqueeze(0)
    ...
    def evaluate_batch(self, obs_seq: torch.Tensor, actions, actor_critic: ActorCritic):
        """
        obs:     (B, S, 3, H, W)
    ...
    def ppo_update(self, buffer: RolloutBuffer, actor_critic: ActorCritic):
        ...
        - Transformer actor (takes obs sequences: (rollout_length, 3, H, W))
        - CNN critic (uses only last frame embedding)
        """
        T = len(buffer)
        img_dim = buffer.obs[0].shape
        obs = torch.stack(buffer.obs).to(DEVICE).view(MINIBATCHES, T // MINIBATCHES, *img_dim)

        actions = torch.tensor(buffer.actions, dtype=torch.long, device=DEVICE).view(MINIBATCHES, T // MINIBATCHES)  # (T,)
